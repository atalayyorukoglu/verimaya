...
describe('calculateRealRoas golden scenarios', () => {
	it('profitable: revenue 3x spend with CPL and cost per treated', () => {
		...
		expect(result.costPerTreated).toBe(20_000);
	});
	...
	it('zero spend: real ROAS null, unit costs are 0 when denominators exist', () => {
		...
		expect(result.costPerTreated).toBe(0);
	});
	...
	it('spend with no conversions: ROAS 0, unit costs null', () => {
		...
		expect(result.costPerTreated).toBeNull();
	});
});
