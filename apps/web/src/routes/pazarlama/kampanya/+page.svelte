<script lang="ts">
	...
	const gaps = $derived.by(() => {
		...
		if (compliance.tone !== 'ok') {
			items.push({ label: 'Uyumluluk', href: '/pazarlama/uyumluluk' });
		}
		if (economics.tone !== 'ok') {
			items.push({ label: 'Hesap', href: '/pazarlama/hesap' });
		}
		if (measurement.tone !== 'ok') {
			items.push({ label: 'Ölçüm', href: '/pazarlama/olcum' });
		}
		return items;
	});
...
<div class="mx-auto max-w-5xl min-w-0">
	...
	<div class="grid gap-4 lg:grid-cols-3">
		<section class="rounded-lg border bg-surface p-5 {panelClass(compliance.tone)}">
			...
			<a href="/pazarlama/uyumluluk" class="mt-3 inline-block text-xs font-medium text-brand hover:underline">
		...
		<section class="rounded-lg border bg-surface p-5 {panelClass(economics.tone)}">
			...
			<a href="/pazarlama/hesap" class="mt-3 inline-block text-xs font-medium text-brand hover:underline">
				Hesap aracına git
			</a>
		</section>
		...
		<section class="rounded-lg border bg-surface p-5 {panelClass(measurement.tone)}">
			...
			{/if}
			<a href="/pazarlama/olcum" class="mt-3 inline-block text-xs font-medium text-brand hover:underline">
				Ölçümü düzenle
			</a>
		</section>
	</div>
