	...
	{:else}
				...
				<ul class="mt-3 divide-y divide-border">
					{#each report.missingLink as t (t.id)}
						<li class="flex justify-between gap-2 py-2 text-sm">
							...
							<a href="/finans" class="shrink-0 text-xs text-brand hover:underline">Düzelt</a>
						</li>
					{/each}
				</ul>
			{/if}
		...
		<section class="mt-4 rounded-lg border border-border bg-surface p-4">
			...
			<div class="mt-3 flex flex-wrap gap-3 text-sm">
				<a href="/kisiler/cift-kayit" class="text-brand hover:underline">Kişilerde tara →</a>
				<a href="/hastalar/cift-kayit" class="text-brand hover:underline">Hastalarda tara →</a>
			</div>
		</section>

		<p class="mt-3 text-xs text-text-faint">
			<a href="/raporlar" class="text-brand hover:underline">Raporlar → tutarlılık</a> ile örtüşür;
