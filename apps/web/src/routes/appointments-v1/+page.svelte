...
<svelte:head>
	<title>{t('appointments.v1.documentTitle')}</title>
</svelte:head>

<div class="mx-auto max-w-6xl min-w-0">
	<div
		class="mb-4 rounded-[6px] border border-border bg-surface-2/50 px-3 py-2 text-sm text-text-muted"
	>
		{t('appointments.v1.archiveBanner')}
		<a href={resolve('/appointments')} class="ml-1 font-medium text-brand hover:underline"
			>{t('appointments.v1.backToCurrent')}</a
		>
	</div>
	<PageHeader title={t('appointments.v1.title')} description={t('appointments.description')}>
		{#snippet actions()}
