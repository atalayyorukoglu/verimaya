<script lang="ts">
	...
	import { resolveDevPanelRoute } from '$lib/dev-panel';
	...
	const routeGate = resolveDevPanelRoute(DEV_PANEL_ENABLED);
	...
	$effect(() => {
		if (!DEV_PANEL_ENABLED || hydratedTarget || tenants.length === 0) return;
	...
	const usersQuery = createQuery(() => ({
		...
		enabled: DEV_PANEL_ENABLED && !!targetTenantId
	}));
...
{:else}
	<div class="mx-auto max-w-3xl min-w-0 space-y-8">
		...
		<p class="text-xs text-text-faint">
			Tracker: <code class="text-text-muted">/dev-users</code> — yalnızca platform dev e-postası. Veri
			Maya’da gerçek erişim Faz 0b’de süper-admin / allowlist ile sınırlanır.
		</p>
	</div>
{/if}
