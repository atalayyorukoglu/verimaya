 ...
 * services (`apps/api/src/{appointments,transactions,contacts}/*.service.ts`)
...
describe('CONTRACT-02: MSW list endpoints match the shared filter + order contract', () => {
	...
	it('appointments: contact_id filter matches the store exactly, newest first', async () => {
		...
		const page = await fetchPage<{ id: string }>(
			`/v1/appointments?contact_id=${ATALAY_CONTACT_ID}&limit=100`
		);
	...
	it('appointments: type_counts/status_counts match the filtered set (GAP-F09-21)', async () => {
		...
		>(`/v1/appointments?contact_id=${ATALAY_CONTACT_ID}&limit=1`);
	...
	it('transactions: contact_id + cursor pagination covers every matching row exactly once', async () => {
		const expected = store.transactions
			.filter((t) => t.contact_id === ATALAY_CONTACT_ID)
			.sort(compareByOccurredOnDesc)
			.map((t) => t.id);
		expect(expected.length).toBeGreaterThan(0);
		const all = await fetchAllPages<{ id: string }>(
			`/v1/transactions?contact_id=${ATALAY_CONTACT_ID}`,
			2
		);
	...
	it('contacts: q filter matches the store exactly', async () => {
		const q = 'atalay';
		const expected = store.contacts
			.filter(
				(c) =>
	...
	it('every list endpoint rejects an unknown query parameter with 400', async () => {
		for (const path of ['/v1/appointments', '/v1/transactions', '/v1/contacts']) {
