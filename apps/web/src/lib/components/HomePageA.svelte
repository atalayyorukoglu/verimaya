<script lang="ts">
	...
	const description =
		'Hasta geldikten sonrasını tek yerde tutar: randevu, tedavi, tahsilat, hakediş, teşvik. Sonra listeyi kendisi çıkarır: kim ödemedi, hangi hekimin oranları bozuldu, kime aylardır dokunulmadı.';
	...
	/**
	 * Defterin satırları. **Hepsi `bizde: true`** çünkü ürün gerçekten beşini de yazıyor:
	 * kişi kartı, randevu, tahsilat/hakediş/teşvik, olay kaydı ve bunların üstündeki rapor.
	 * Basamakların bir kısmını "başkası yapar" diye işaretlemek kodun yaptığı işi küçültürdü.
	 */
	const ledger = [
		{
			no: '1',
		...
		},
		{
			no: '2',
			ad: 'Randevu',
			soru: 'Ne zaman, hangi hekimle, adımı onaylandı mı?',
			kim: 'randevu · operasyon adımları',
			bizde: true
		},
		{
			no: '3',
			...
			kim: 'tahsilat · hakediş · teşvik',
			bizde: true
		},
		{
			no: '4',
			ad: 'Olay',
			soru: 'Ne aksadı, kim ilgilendi, kapandı mı?',
			kim: 'olay kaydı',
			bizde: true
		},
		{
			no: '5',
			ad: 'Teşhis',
			soru: 'Neden böyle oldu, bu ay kime bakılacak?',
			kim: 'rapor · müdahale listesi',
			bizde: true
		}
	] as const;

	/**
	 * Yalnız üründe karşılığı olan sorular. Reklam, lead dönüş süresi ve satış sohbeti
	 * bilerek yok: `contacts.campaign` kolonu hiçbir rapora girmiyor, dönüş süresi metriği
	 * hiç yazılmadı ve WhatsApp modülü satış konuşmasını değil tutar/randevu bilgisini
	 * okuyor. Vaat edilmeyen şey burada durmaz.
	 */
	const reads = [
	 ...
	 * etiketi o kural, yanındaki rakam da bulgunun dayandığı kayıt sayısı. Eşiğin altında
	 * kalan hiçbir değişim listeye girmediği için "dayanak" alanı boş kalmaz.
	 */
	const actions = [
		{
	...
	/**
	 * Eski döngü metni "müdahale işe yaradı mı?" diye soruyordu; üründe bir müdahaleyi
	 * kapatma ya da sonucunu işaretleme alanı yok. Bu yüzden dördüncü adım, sistemin
	 * gerçekten yaptığı şeye indirildi: ertesi dönem aynı rakamı yeniden hesaplamak.
	 */
	const cycle = [
		{
			no: '01',
			ad: 'Yaz',
			not: 'Kayıt, randevu, para ve olay aynı deftere düşer.',
			ornek: 'Örn. 42 randevu · 18 tahsilat · 9 açık olay'
		},
		{
			no: '02',
			ad: 'Ölç',
			not: 'Dönem rakamları ve önceki dönemle farkı çıkar.',
			ornek: 'Örn. RPT %6 → %11 · açık alacak 240 bin'
		},
		{
			no: '03',
			ad: 'Ayıkla',
			not: 'Eşiği geçen değişim listeye girer, gerisi girmez.',
			ornek: 'Örn. 6 değişimden 2’si eşiği geçti'
		},
		{
			no: '04',
			ad: 'Tekrar ölç',
			not: 'Ertesi dönem aynı rakam yine önünüzde olur.',
			ornek: 'Örn. RPT %11 → %7 · açık olay 9 → 3'
		}
	] as const;
	...
	<main>
		...
		<section class="v4-hero">
			...
			<div class="v4-hero-in">
				...
				<h1>Verimaya, <em>“bu ay neyi atladık”</em> sorusunu cevaplar.</h1>
				<p>
					Hasta geldikten sonrasını tek yerde tutar: randevu, tedavi, tahsilat, hakediş, teşvik.
					Sonra listeyi kendisi çıkarır: kim ödemedi, hangi hekimin oranları bozuldu, kime aylardır
					dokunulmadı, hangi dosyanın süresi doluyor.
				</p>
				<div class="v4-cta-row">
					<a class="v4-btn" href="#gorusme">İlk bulgu raporu için görüşme isteyin</a>
					<a class="v4-link" href="#defter">Nasıl çalışır ↓</a>
				</div>
			</div>
		</section>

		<!-- ── 2. Defter ────────────────────────── -->
		<section id="defter" class="v4-band v4-ladder-band">
			<div class="v4-in">
				<span class="v4-label">Omurga</span>
				<h2>Dağınık defterin teşhisi olmaz.</h2>
				<p class="v4-lede">
					Kayıt bir dosyada, randevu bir grupta, para başka bir tabloda duruyorsa “neden böyle oldu”
					sorusu cevaplanamaz. Önce tek defter, sonra teşhis. Verimaya birinci satırdan beşinciye
					kadar aynı deftere yazar.
				</p>

				<ol class="v4-ladder">
					{#each ledger as s (s.no)}
				...
				<p class="v4-note">
					Teşhis, bu beş satırın aynı yerde durmasıyla mümkün oluyor. Ayrı sistemlerde tutulan
		...
		<section id="okuruz" class="v4-band">
			<div class="v4-in">
				...
				<h2>Hasta geldikten sonrasına bakarız.</h2>
				<p class="v4-lede">
					Reklam ve satış hattı başka sistemlerde yaşıyor. Biz onları taklit etmiyoruz; hastanın
					kapıdan girdiği andan sonrasını alıyoruz. Bu bir eksiklik değil, sınır. Sınırı belli olan
					bir sistem, her şeyi bildiğini söyleyen bir sistemden daha kullanışlıdır.
				</p>
		...
		<section class="v4-band">
			<div class="v4-in">
				...
				<h2>Liste kendiliğinden yenilenir.</h2>
				<p class="v4-lede">
					Müdahaleyi siz yaparsınız. Sistemin işi, sonraki dönemde aynı rakamı hiç sormadan yeniden
					önünüze koymak.
				</p>
				<ol class="v4-cycle">
		...
		<section class="v4-band">
			<div class="v4-in v4-aud">
				<article class="v4-aud-main">
					...
					<h2>Hastayı ağırlayan ve parasını takip edenler için.</h2>
					<p>
						Hasta kaydı sizde açılıyor, randevusunu siz kuruyor, tahsilatı siz yapıyor, iş ortağının
						hakedişini siz ödüyorsanız doğru yerdesiniz. Reklamı ve satış hattını hangi sistemde
						yürüttüğünüz değişmez; onu olduğu yerde bırakırsınız.
					</p>
					<p>
						Hasta, randevu, tahsilat, hakediş ve teşvik aynı yerde. Hangi iş ortağının kaç hasta
						getirdiği ve kimin ne hak ettiği tek tabloda.
					</p>
				</article>
				<aside>
