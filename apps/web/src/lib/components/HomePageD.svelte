<script lang="ts">
	...
	/**
	 * C sürümündeki "hangi kanal para kaybettiriyor" iddiası kaldırıldı: harcama yalnız
	 * tek toplam olarak toplanıyor (`sumAdSpend`), kaynak satırı ise harcama taşımıyor
	 * (`marketingSourceRow`). Yani kanal düzeyinde ROAS diye bir çıktı yok.
	 */
	const description =
		'Reklam parasının kasaya ne getirdiğini tahsilattan hesaplar. Hasta geldikten sonrasını aynı deftere yazar ve ayın listesini kendisi çıkarır.';
	 ...
	 * satışı CRM yürütüyor; Verimaya ikisini yalnız okuyor. Kalan üç satır `bizde: true`,
	 * çünkü operasyon, para ve teşhis gerçekten burada yazılıyor.
	 *
	 * Reklam satırında "günlük" yazmıyor: zamanlayıcı altı saatte bir çalışıyor ve
	 * varsayılanı kapalı (`INTEGRATION_SCHEDULER_EVERY_MS`, `ENABLE_INTEGRATION_SCHEDULERS`).
	 * Sıklık sözü vermemek doğrusu.
	 */
	const chain = [
		{
			no: '1',
			...
			kim: 'Google Ads · Meta · düzenli okunur',
			bizde: false
		},
		{
			no: '2',
	 ...
	 *
	 * Kapsama oranı %88 seçildi, C'deki %71 değil: `ATTRIBUTION_COVERAGE_THRESHOLD = 0.8`
	 * altında ürün ROAS'ı hiç yayınlamıyor, yani %71'lik bir örnek yanında ROAS göstermek
	 * ürünün kendi kuralını çiğniyordu.
	 */
	const roas = [
		{ k: 'Reklam harcaması', v: '412.000 ₺', n: 'Google Ads + Meta, dönemin toplamı' },
	...
	/**
	 * Verinin okunduğu halkalar. Reklam satırı tek toplam soruyor; kaynak satırı ise
	 * yalnız gelir tarafını soruyor, çünkü kaynak kırılımında harcama yok.
	 */
	const reads = [
		{ halka: 'Reklam', soru: 'Google ve Meta’ya hangi gün ne kadar harcandı?' },
		{ halka: 'Kaynak', soru: 'Hangi kaynaktan gelen hasta ne kadar ödedi?' },
	...
	/**
	 * Müdahale listesi bugün dört kural işletiyor (`quality_drop`, `revenue_drop`,
	 * `open_incident`, `referral_value`). Referans satırı "tahsilat" diyor, "hasta" değil:
	 * `buildReferralValue` listeyi `total_net_base` azalanına göre kesiyor.
	 */
	const actions = [
		{
			baslik: 'Dr. Kaya · RPT oranı %6’dan %11’e çıktı',
			neden: 'Önceki dönemle karşılaştırıldı',
			kural: 'kalite',
			dayanak: '42 randevu'
		},
		{
		...
		{
			baslik: 'En çok tahsilat getiren 5 referans',
	...
	/**
	 * Okuyucu sağlık turizmi acentesi sahibi, yazılımcı değil. C'deki "şablon", "eşik",
	 * "tenant", "veritabanı" sözcükleri bu yüzden atıldı; anlatılan mekanizma aynı kaldı.
	 */
	const kural = [
		{
			b: 'Cümleler hazır',
			t: '“Dr. Kaya’nın RPT oranı %6’dan %11’e çıktı” cümlesinin kalıbı önceden yazılmıştır. Yapay zekâ o cümleyi kurmaz, yalnız boşlukları doldurur.'
		},
		{
			b: 'Rakamlar sizin',
			t: 'Her sayı sizin girdiğiniz kayıttan gelir. Tahmin yok, yuvarlama yok, örnekleme yok.'
		},
		{
			b: 'Ölçü sabit',
			t: 'Bir bulgunun ne zaman haber sayılacağı değişmez. Bir ay %5, ertesi ay %9 diye kayan bir şey yoktur.'
		},
		{
			b: 'Sayabilirsiniz',
			t: 'Bulgunun dayandığı kayıtlara inip aynı sayıyı elle sayabilirsiniz.'
		}
	] as const;

	/**
	 * C'deki "destek için verinize erişirsek denetim kaydınıza düşer, sessiz erişim yok"
	 * cümlesi silindi: öyle bir destek erişimi özelliği de, onun kaydı da üründe yok.
	 */
	const privacy = [
		{
			b: 'İzolasyon',
		...
		{
			b: 'Şeffaflık',
			t: 'Aynı sektörde birden fazla firmayla çalışıyoruz; bunu siz sormadan söylüyoruz.'
		}
	] as const;

	/**
	 * Eksik listesi altıdan üçe indi. Çıkanlar: "satış hattı" ve "sohbet analizi" bu
	 * bölümün lede'sine girdi, "müdahale takibi" ise zaten müdahale bölümünde söyleniyor.
	 */
	const yol = [
		{
			halka: 'Kampanya kırılımı',
			soru: 'Bugün tek bir toplam ROAS çıkıyor. Hangi kampanyanın ya da kanalın parayı getirdiği ayrı ayrı görünmüyor.'
		},
		{
			halka: 'Dönüş süresi',
			soru: 'Bir lead’e kaç saatte dönüldüğü ölçülmüyor. Böyle bir ölçü yazılmadı.'
		},
		{
			halka: 'Giden mesaj',
...
>
	...
	<main>
		...
		<section class="v4-hero">
			...
			<div class="v4-hero-in">
				...
				<p>
					...
					çıkarır: kim ödemedi, hangi hekimin oranları bozuldu, kime aylardır dokunulmadı.
				</p>
		...
		<!-- 2. Kim neyi yapıyor. Sınır en başta dursun ki okuyucu ROAS iddiasını sınırı
		     bilerek okusun. C'deki "Omurga" bölümü buradan kaldırıldı: aynı şekilde çizilmiş
		     ikinci bir beş satırlık listeydi. Tek değerli cümlesi aşağıdaki nota taşındı. -->
		<section id="zincir" class="v4-band v4-ladder-band">
			<div class="v4-in">
				...
				<p class="v4-lede">
					...
					hesabınızdan düzenli çeker, kişiyi CRM’den okur, geri kalanını kendi kaydeder.
				</p>
				...
				<p class="v4-note">
					...
					satır Verimaya’nın kendi defteri. Teşhis ancak defter tekse mümkün: ayrı sistemlerde duran
					rakamlar birbirini doğrulayamaz.
				</p>
			</div>
		</section>

		<!-- 3. Gerçek ROAS. Ürünün en ayırt edici kanıtı, bu yüzden üst yarıda duruyor. -->
		<section id="roas" class="v4-band v4-tint">
			<div class="v4-in v4-two">
				<div>
					...
					<p class="v4-lede">
						Verimaya harcamayı <b>kasaya giren paraya</b> böler. Çıkan rakam dönemin toplamıdır: bütün
						reklam harcaması, bütün tahsilat. Yanında hasta başı maliyet durur.
					</p>
					<p class="v4-note">
				...
				<div class="v4-surface">
					...
					<p class="v4-roas-warn">84 hasta dosyasının %88’inde kaynak yazılı.</p>
				</div>
			</div>

			<!-- Sayfada cevapsız kalan itiraz buydu: rakamın çalışması için kaynağın birinin
			     yazması gerekiyor. Saklamak yerine söylüyoruz. -->
			<div class="v4-in">
				<p class="v4-callout">
					Bu rakam kaynak bilgisine bağlı. Dosyalarınızın en az beşte dördünde hastanın nereden
					geldiği yazılı olmalı; altına düşerse ROAS’ı hiç yayınlamayız. Bugün bu bilgi panelde,
					hasta kaydı açılırken seçiliyor.
				</p>
			</div>
		</section>

		<!-- 4. Teşhisin girdileri -->
		<section id="okuruz" class="v4-band">
			<div class="v4-in">
				<span class="v4-label">Teşhisin girdileri</span>
				<h2>Harcamadan hakedişe kadar aynı sorular.</h2>
				<p class="v4-lede">
					Reklam tarafında tek toplam bakarız: bütün harcama, bütün tahsilat. Hasta geldikten
					sonrasında ise kayıt bizde olduğu için soru sayısı artar.
				</p>
				...
				<p class="v4-note">Kaynağı bilinmeyen dosya, bilinen gibi gösterilmez.</p>
			</div>
		</section>
			...
			<div class="v4-in v4-two">
				<div>
					...
					<p class="v4-lede">
						...
						açık kaldıysa, en çok tahsilat getiren referanslar belliyse. Her satırda ne olduğu ve
						kaç kayda dayandığı yazar.
					</p>
					<p class="v4-lede">
					...
					<p class="v4-note">
						Kimin neyi yaptığını sistem takip etmiyor. Liste çıkar, müdahaleyi siz yaparsınız,
						ertesi dönem aynı rakam yine önünüze gelir.
					</p>
				</div>
		...
		<!-- 6. Girdi kalitesi. Örnek kartta mesajın yazarı görünmüyor: öneri kaydı yalnız
		     mesajdan alınan metni, onaylayanı ve onay saatini tutuyor. -->
			...
			<div class="v4-in v4-two v4-two-rev">
				<div class="v4-surface">
					...
					<div class="v4-q-item">
						...
						<p class="v4-src">“Ahmet Bey 12 Eylül’e ertelemek istedi”</p>
				...
				<div>
					...
					<p class="v4-lede">
						Ameliyat sonrası hâlâ WhatsApp’ta konuşuluyor. Bu yüzden veri girişini insana yük
						olmaktan çıkardık: ekibiniz ne yazıyorsa sistem onu okur, değişikliği hazırlar, siz
						onaylarsınız.
						<b>Onaylayana kadar hiçbir kayıt değişmez.</b>
					</p>
					<p class="v4-lede">
						Her önerinin yanında mesajdan alınan metin durur. Onayladığınızda kimin onayladığı ve ne
						zaman onayladığı kaydedilir.
					</p>
					<p class="v4-note">
		...
		<!-- 7. Güven. Okuyucu acente sahibi; bu bölümde teknik terim kullanmıyoruz. -->
		<section class="v4-band v4-tint">
			<div class="v4-in">
				<span class="v4-label">Güven</span>
				<h2>Yapay zekâ rakam uydurmaz.</h2>
				<p class="v4-lede">
					Size gösterilen her bulgunun nereden geldiği bellidir. Sorarsanız açar, gösteririz.
				</p>
				<div class="v4-grid4">
					{#each kural as k (k.b)}
						<article>
		...
		<!-- 9. Kimin için + kapanış tek bölümde. C'de ayrı duruyorlardı ve "ücret
		     istemiyoruz" sözü iki kez geçiyordu; bir kez kaldı. -->
		<section id="gorusme" class="v4-closing">
			<div class="v4-closing-in">
				...
				<p>
					...
					tahsilatı siz yapıyorsanız doğru yerdesiniz. Hiçbirini bırakmanız gerekmiyor; hepsi olduğu
					yerde kalır. Tek marka ile de çalışır, birden fazla klinik ve iş ortağıyla da.
				</p>
				<p>
					30 dakikalık bir görüşme yapalım. Uygunsa sözleşmeyle veriyi alır, doğrularız. Doğrulama
					biter bitmez dört şey görünür: reklam harcamanız, o paranın getirdiği tahsilat, kaç
					hastaya 30 günden uzun süredir dokunulmamış, ne kadar bakiye tahsil edilmemiş.
				</p>
		...
		<!-- 10. Sırada ne var. Bilerek kapanıştan sonra: eksik listesi çağrının önüne
		     geçmesin ama demoda sürpriz de olmasın. -->
		<section id="sirada" class="v4-band v4-tint v4-band-top">
			<div class="v4-in">
				<span class="v4-label">Sırada ne var</span>
				<h2>Bunlar bugün yok. Sözünü de vermiyoruz.</h2>
				<p class="v4-lede">
					Aşağıdakiler demoda sorulursa cevabı “henüz yok” olur. Bir örnek: web sitesi formundan
					gelen hastanın kaynağı CRM’de zaten duruyor, biz henüz oradan almıyoruz.
				</p>
...
<style>
	...
	.v4-hero p {
		...
		max-width: 40rem;
		/* .v4-lede ile aynı boy: sayfa boyunca tek gövde ölçüsü. */
	...
	/* "Sırada ne var" kapanıştan sonra geliyor; kapanışın alt kenarlığı olmadığı için
	   ayrımı bu bant kendi üstünden çiziyor. */
	.v4-band-top {
	...
	/* Kaynak itirazının cevabı. Nota göre daha görünür olmalı: sayfanın en sık sorulan
	   sorusu bu ve cevabı "bunu siz gireceksiniz". */
	.v4-callout {
		margin: 2.5rem 0 0;
	...
	/* Üçüncü satır ROAS; sayfanın asıl iddiası olduğu için ayrıca vurgulanıyor. */
