<script lang="ts">
	...
	const formDescription = $derived(
		USE_MSW ? t('patients.form.descriptionDemo') : t('patients.form.description')
	);
...
>
	<form id="patient-form" class="space-y-3" onsubmit={handleSubmit}>
		<div>
			<label class={labelClass} for="patient-name">{t('patients.form.name')}</label>
	...
	{#snippet footer()}
		...
		<Button type="submit" form="patient-form" disabled={saving || !full_name.trim()}>
			{saving ? t('patients.form.saving') : isEdit ? t('patients.form.save') : t('patients.form.create')}
		</Button>
	{/snippet}
</Dialog>
