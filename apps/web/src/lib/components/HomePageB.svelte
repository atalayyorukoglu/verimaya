<script lang="ts">
	...
	const title = 'Verimaya · Reklamdan tahsilata tek defter';
	const description =
		'Google ve Meta harcamasını okur, hastanın ödediği parayla karşılaştırır. Teklif edilen tutarla değil, kasaya girenle. Sonra randevu, tedavi, tahsilat, hakediş ve teşviki aynı deftere yazar.';
	...
	/**
	 * Zincirin ilk iki satırı `bizde: false`, çünkü reklamı Google ve Meta, satışı GHL
	 * yürütüyor; Verimaya ikisini yalnız **okuyor**. Kalan üç satır `bizde: true`, çünkü
	 * operasyon, para ve teşhis gerçekten burada yazılıyor. Beşini de sahiplenmek ya da
	 * ilk ikisini hiç anmamak, ekranda görülen şeyle uyuşmazdı.
	 */
	const chain = [
		{
			no: '1',
		...
		},
		{
			no: '5',
			ad: 'Teşhis',
			soru: 'Reklam parası kasaya ne getirdi, bu ay kime bakılacak?',
			kim: 'Verimaya raporu',
			bizde: true
		}
	] as const;
	 ...
	 * paydayı `resolveCollectedAmount` kuruyor, kesilen fatura değil. Kapsama satırı da
	 * süs değil, üründe gerçekten basılan uyarının kendisi.
	 */
	const roas = [
	 ...
	 * yazılamaz. Sınırı burada söylemek, raporda boş sütun görmekten iyi.
	 */
	const reads = [
	...
	/**
	 * Müdahale listesi bugün tam olarak dört kural işletiyor (`quality_drop`,
	 * `revenue_drop`, `open_incident`, `referral_value`). Etiketler o kurallar; reklam
	 * tarafı listeye girmiyor, çünkü orada bir eşik kuralı yazılmadı.
	 */
	const actions = [
		{
	...
	/**
	 * Dördüncü adım "işe yaradı mı" demiyor: müdahaleyi kapatma ya da sonucunu işaretleme
	 * alanı üründe yok. Sistemin gerçekten yaptığı şey, ertesi dönem aynı rakamı yeniden
	 * hesaplamak.
	 */
	const cycle = [
		{
			no: '01',
			ad: 'Çek',
			not: 'Google, Meta ve GHL kendi tarafından okunur.',
			ornek: 'Örn. 62 gün harcama · 340 kişi'
		},
		{
			no: '02',
			ad: 'Yaz',
			not: 'Randevu, para ve olay aynı deftere düşer.',
			ornek: 'Örn. 42 randevu · 18 tahsilat · 9 açık olay'
		},
		{
			no: '03',
			ad: 'Böl',
			not: 'Harcama, kasaya giren paraya bölünür.',
			ornek: 'Örn. 412 bin ÷ 1,24 milyon → 3,0'
		},
		{
			no: '04',
			ad: 'Tekrar ölç',
			not: 'Ertesi dönem aynı rakam yine önünüzde olur.',
			ornek: 'Örn. ROAS 3,0 → 2,4 · açık olay 9 → 3'
		}
	] as const;
	...
	const yol = [
		...
		{
			halka: 'Satış hattı',
			soru: 'GHL’den bugün yalnız kişi alınıyor: isim, telefon, e-posta. Etiket, temsilci ve hat aşaması çekilmiyor.'
		},
		{
	...
	<main>
		...
		<section class="v4-hero">
			...
			<div class="v4-hero-in">
				...
				<h1>Reklam parası kasaya ne getirdi? Verimaya cevabı <em>tahsilattan</em> hesaplar.</h1>
				<p>
					Google ve Meta harcamasını okur, hastanın ödediği parayla karşılaştırır. Teklif edilen
					tutarla değil, kasaya girenle. Sonra hasta geldikten sonrasını da aynı deftere yazar:
					randevu, tedavi, tahsilat, hakediş, teşvik.
				</p>
		...
		<section id="zincir" class="v4-band v4-ladder-band">
			<div class="v4-in">
				...
				<h2>Satışını GHL yapar, reklamını Google ve Meta. Defteri Verimaya tutar.</h2>
				<p class="v4-lede">
					Verimaya bunların yerine geçmiyor, üstüne oturuyor. Reklam harcamasını Google Ads ve Meta
					hesabınızdan günlük çeker, kişiyi GHL’den okur, geri kalanını kendi kaydeder. Beş satır
		...
		<section id="roas" class="v4-band v4-tint">
			<div class="v4-in v4-two">
				<div>
					...
					<h2>Payda tahsilat. Kesilen fatura değil.</h2>
					<p class="v4-lede">
						Çoğu yerde ROAS, reklam harcamasının <b>teklif edilen</b> ya da faturalanan tutara bölünmesiyle
						çıkar. Sağlık turizminde bu rakam yanıltır: teklif verilir, hasta gelmez; fatura kesilir,
						bakiye aylarca kapanmaz.
					</p>
					<p class="v4-lede">
						Verimaya harcamayı <b>kasaya giren paraya</b> böler. Aynı ekranda hasta başı maliyet de durur.
					</p>
					<p class="v4-note">
						Kur bilgisi eksikse ROAS hiç yayınlanmaz, alan boş kalır. Yanlış rakam basmaktansa
		...
		<section class="v4-band">
			<div class="v4-in">
				...
				<h2>Rakam kendiliğinden yenilenir.</h2>
				<p class="v4-lede">
			...
			<div class="v4-in v4-two v4-two-rev">
				...
				<div>
					...
					<p class="v4-lede">
						...
						ne zaman. Evrak istersen Google Drive’a aynalanır.
					</p>
					<p class="v4-note">
		...
		<!-- ── 11. Sırada ne var ───────────────── -->
		<section id="sirada" class="v4-band v4-tint">
			<div class="v4-in">
		...
		<section class="v4-band">
			<div class="v4-in v4-aud">
				<article class="v4-aud-main">
					...
					<p>
						Google ve Meta’ya para veriyor, satışı GHL’de yürütüyor, hasta kaydını kendiniz açıyor,
