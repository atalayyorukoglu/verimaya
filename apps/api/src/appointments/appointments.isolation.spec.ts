...
describe("appointments tenant isolation", () => {
  ...
  it("hekim alanı: doctor_contact_id nullable — eski/hekimsiz randevu bozulmadan çalışır", async () => {
    ...
    const found = (
      await appointmentsService.list(tenantA, { limit: 100 })
    ).items.find((a) => a.id === noDoctorId);
    expect(found?.doctor_contact_id ?? null).toBeNull();
  });

  it("lojistik adları: kolon boş olsa bile klinik/otel/transfer adı kişiden türetilir", async () => {
    // 2026-09-08 öncesi panel `clinic_name: null` gönderiyordu; kimlik kaldı, ad
    // silindi ve randevu kartı "—" gösteriyordu (Linda Wilson, 26.09.2026).
    // Ad artık okuma anında join'le geliyor, eski satırlar da düzgün görünüyor.
    const [clinicId, hotelId, transferId] = await withTenantSession(
      tenantA,
      async (tdb) => {
        const clinic = await contactsService.createWithDb(tdb, tenantA, {
          contact_type_id: contactTypeA,
          first_name: "Klinik Ada",
        });
        const hotel = await contactsService.createWithDb(tdb, tenantA, {
          contact_type_id: contactTypeA,
          first_name: "Otel Beta",
        });
        const transfer = await contactsService.createWithDb(tdb, tenantA, {
          contact_type_id: contactTypeA,
          first_name: "Transfer Gama",
        });
        return [clinic.id, hotel.id, transfer.id];
      },
    );
    ...
    // Eski bozuk satırı birebir taklit et: kimlikler dursun, adlar boşalsın.
    await withTenantSession(tenantA, async (tdb) => {
      await tdb
        .update(appointments)
        .set({ clinicName: null, hotelName: null })
        .where(eq(appointments.id, apptId));
    });

    const found = (
      await appointmentsService.list(tenantA, { limit: 100 })
    ).items.find((a) => a.id === apptId);
    expect(found?.clinic_name).toBe("Klinik Ada");
    expect(found?.hotel_name).toBe("Otel Beta");
    expect(found?.transfer_contact_name).toBe("Transfer Gama");
  });
  ...
  it("hekim alanı: hekim kişisi silinince (hard delete) randevu düşmez, doctor_contact_id null olur (ON DELETE SET NULL)", async () => {
    ...
    await withTenantSession(tenantA, async (tdb) => {
      await tdb.execute(
        drizzleSql`delete from contacts where id = ${doctorId}`,
      );
    });
  ...
  it("Tenant A cannot update Tenant B appointment", async () => {
    await withTenantSession(tenantA, async (tdb) => {
      await expect(
        appointmentsService.updateWithDb(tdb, appointmentB, {
          title: "Hacked by A",
        }),
    ...
    expect(stillB.items.find((a) => a.id === appointmentB)?.title).toBe(
      "Appointment B",
    );
  });
  ...
  it("GAP-04: status='no_show' returns only matching appointments", async () => {
    ...
    const byStatus = await appointmentsService.list(tenantA, {
      limit: 25,
      status: "no_show",
    });
  ...
  it("GAP-04: q matches patient display name substring only", async () => {
    const searchablePatientId = await withTenantSession(
      tenantA,
      async (tdb) => {
        const p = await contactsService.createWithDb(tdb, tenantA, {
          contact_type_id: contactTypeA,
    ...
    const byQ = await appointmentsService.list(tenantA, {
      limit: 25,
      q: "UniqueGap04",
    });
  ...
  it("GAP-F09-21: type_counts/status_counts over filtered set; cursor and soft-delete ignored", async () => {
    ...
    const page1 = await appointmentsService.list(tenantA, {
      limit: 1,
      q: marker,
    });
  ...
  it("GAP-F09-21: Tenant B aggregates exclude Tenant A's appointments", async () => {
    ...
    const tenantATotal = Object.values(listA.status_counts).reduce(
      (s, n) => s + n,
      0,
    );
  ...
  it("G-29: derives contact_info_incomplete on create and list without cross-tenant leakage", async () => {
    ...
    const [completeContactId, incompleteContactId, phoneOnlyContactId] =
      await withTenantSession(tenantA, async (tdb) => {
        const completeContact = await contactsService.createWithDb(
          tdb,
          tenantA,
          {
            contact_type_id: contactTypeA,
        ...
        const incompleteContact = await contactsService.createWithDb(
          tdb,
          tenantA,
          {
            contact_type_id: contactTypeA,
        ...
        const phoneOnlyContact = await contactsService.createWithDb(
          tdb,
          tenantA,
          {
            contact_type_id: contactTypeA,
    ...
    const listA = await appointmentsService.list(tenantA, {
      limit: 25,
      q: marker,
    });
    ...
    const listB = await appointmentsService.list(tenantB, { limit: 25 });
    expect(
      listB.items.some((appointment) => appointment.id === incomplete.id),
    ).toBe(false);
  });
  ...
  it("liste ziyaret tarihine göre sıralanır, cursor da aynı sütunu taşır", async () => {
    ...
    const page = await appointmentsService.list(tenantA, {
      limit: 25,
      q: marker,
    });
