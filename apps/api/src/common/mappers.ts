...
export function toAppointment(
	row: AppointmentRow,
	extras?: {
		contact_info_incomplete?: boolean;
		/** Kişiden okuma anında türetilen adlar; kayıtlı ad boşsa onun yerine geçer. */
		clinic_contact_name?: string | null;
		hotel_contact_name?: string | null;
		transfer_contact_name?: string | null;
	}
): Appointment {
	return {
		id: row.id,
		...
		// Kimlik varsa ad HER ZAMAN kişiden gelir. Kolondaki değer yalnızca serbest
		// metin girişleri için (kimlik yokken) ve arama için tutulur; 2026-09-08
		// öncesi kaydedilen satırlarda boş kalmıştı, kart "—" gösteriyordu.
		clinic_name: extras?.clinic_contact_name ?? row.clinicName,
		hotel_name: extras?.hotel_contact_name ?? row.hotelName,
		...
		transfer_contact_name: extras?.transfer_contact_name ?? null,
...
export function toIncentiveFile(
	row: IncentiveFileRow,
	today: string = utcTodayIsoDate()
): IncentiveFile {
...
export function toRecordUpdateSuggestion(
	...
	return {
		...
		current_value: row.currentValue ? toIsoDateTime(row.currentValue) : null,
		suggested_value: row.suggestedValue ? toIsoDateTime(row.suggestedValue) : null,
		current_text: row.currentText,
		suggested_text: row.suggestedText,
		suggested_contact_id: row.suggestedContactId,
...
export function toTransaction(
	...
	return {
		...
		contact_visit_id: row.contactVisitId ?? null,
...
export function toWhatsappChat(row: WhatsappChatRow, messageCount = 0): WhatsappChat {
	return {
		id: row.id,
		tenant_id: row.tenantId,
		chat_id: row.chatId,
...
export function toTenant(row: TenantRow, baseCurrencyLocked = false): Tenant {
	return {
		...
		data_retention_until: row.dataRetentionUntil ? toIsoDateTime(row.dataRetentionUntil) : null,
		created_at: toIsoDateTime(row.createdAt)
	};
}
