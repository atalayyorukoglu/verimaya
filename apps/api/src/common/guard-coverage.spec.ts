...
import { AdsController, AdsOAuthCallbackController } from '../integrations/ads/ads.controller';
import { GhlController, GhlOAuthCallbackController } from '../integrations/ghl/ghl.controller';
...
const PUBLIC_ROUTE_ALLOWLIST: Record<string, string> = {
	...
	'KarneController.createSession': 'Public marketing karne funnel — unauthenticated writes to karne_* only',
	'KarneController.createEvent': 'Public karne analytics event',
	'KarneController.complete': 'Public karne completion',
	'KarneController.createLead': 'Public karne lead capture (LEG-02 gated by env)',
...
describe('AUDIT-F09-04: every route has the org-auth guard triad or an allowlisted reason', () => {
	...
	it('allowlist entries reference real handlers and every entry has a non-empty reason', () => {
		...
		for (const [key, reason] of Object.entries(PUBLIC_ROUTE_ALLOWLIST)) {
			expect(routeKeys.has(key), `allowlist key missing from scan: ${key}`).toBe(true);
	...
	it('every non-allowlisted route carries AuthOrApiKey|Session + ActiveOrg + OrgPermission', () => {
		...
		for (const route of routes) {
			...
			const controller = ALL_CONTROLLERS.find((c) => c.name === route.controller)!;
			const handler = (controller.prototype as Record<string, Function>)[route.handler];
	...
	it('no stale allowlist keys (every allowlisted handler still exists)', () => {
		const routeKeys = new Set(routes.map((r) => r.key));
		const stale = Object.keys(PUBLIC_ROUTE_ALLOWLIST).filter((k) => !routeKeys.has(k));
...
describe('AUDIT-F09-18: class-level guard is the dominant / required pattern for org controllers', () => {
	it('AdsController and GhlController declare the triad at class level (not per-method)', () => {
		...
		for (const method of ['status', 'authorize', 'disconnect'] as const) {
			expect(Reflect.getMetadata(GUARDS_METADATA, AdsController.prototype[method])).toBeUndefined();
			expect(Reflect.getMetadata(GUARDS_METADATA, GhlController.prototype[method])).toBeUndefined();
		}
		expect(
			Reflect.getMetadata(GUARDS_METADATA, AdsController.prototype.updateGoogleCustomerId)
		).toBeUndefined();
	});

	it('OAuth callback controllers remain unguarded at class and method level', () => {
		expect(Reflect.getMetadata(GUARDS_METADATA, AdsOAuthCallbackController)).toBeUndefined();
		expect(
			Reflect.getMetadata(GUARDS_METADATA, AdsOAuthCallbackController.prototype.callback)
		).toBeUndefined();
		expect(Reflect.getMetadata(GUARDS_METADATA, GhlOAuthCallbackController)).toBeUndefined();
		expect(
			Reflect.getMetadata(GUARDS_METADATA, GhlOAuthCallbackController.prototype.callback)
		).toBeUndefined();
	});
});
