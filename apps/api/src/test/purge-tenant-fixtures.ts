...
export async function purgeTenantFixtures(sql: Sql, tenantIds: string[]): Promise<void> {
	...
	for (const tenantId of tenantIds) {
		await sql.begin(async (tx) => {
			...
			await tx`delete from contact_titles where tenant_id = ${tenantId}`;
			...
			await tx`delete from maya_questions where tenant_id = ${tenantId}`;
			await tx`delete from whatsapp_chats where tenant_id = ${tenantId}`;
		});
	}
