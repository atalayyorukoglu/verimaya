...
const bundleDryRunQuerySchema = z
	.object({
		scope: bundleSheetSchema.optional()
	})
	.strict();
...
export class ImportExportController {
	...
	@Get('bundle/template.xlsx')
	@RequireOrgPermission('settings', 'read')
	@IdempotencyExempt('Binary template download — no mutation.')
	async bundleTemplate(@Res({ passthrough: true }) reply: FastifyReply) {
		const file = await this.importExport.bundleTemplate();
		reply.header('Content-Type', file.mimeType);
		reply.header('Content-Disposition', contentDispositionHeader('attachment', file.filename));
		reply.header('X-Content-Type-Options', 'nosniff');
		reply.header('Content-Length', String(file.buffer.byteLength));
	...
	@Get('bundle/export.xlsx')
	@RequireOrgPermission('settings', 'read')
	@IdempotencyExempt('Binary export download — no mutation.')
	async bundleExport(@Req() req: FastifyRequest, @Res({ passthrough: true }) reply: FastifyReply) {
		const file = await this.importExport.bundleExport(getActiveOrgId(req));
		reply.header('Content-Type', file.mimeType);
		reply.header('Content-Disposition', contentDispositionHeader('attachment', file.filename));
		reply.header('X-Content-Type-Options', 'nosniff');
		reply.header('Content-Length', String(file.buffer.byteLength));
	...
	@Post('bundle/import/dry-run')
	@RequireOrgPermission('settings', 'update')
	@IdempotencyExempt(
		'Dry-run parses untrusted upload and returns a signed plan; no DB writes — repeat is safe.'
	)
	async bundleDryRun(@Req() req: FastifyRequest, @Query() query: Record<string, unknown>) {
		const { scope } = parseQuery(bundleDryRunQuerySchema, query, req);
		const data = await this.readUpload(req);
		return this.importExport.bundleDryRun(getActiveOrgId(req), data, scope ?? null);
	}

	@Post('bundle/import/commit')
	@RequireOrgPermission('settings', 'update')
	@Idempotent()
	async bundleCommit(
		@Req() req: FastifyRequest,
		...
		const result = await this.idempotency.run(
			...
			getIdempotencyKey(req),
			'POST',
			'/v1/settings/import-export/bundle/import/commit',
			async (db) => ({
				statusCode: 200,
				body: await this.importExport.bundleCommitWithDb(db, tenantId, input.plan_token, actor)
			})
		);
		reply.status(result.statusCode);
		return result.body;
