...
describe('buildLoginAuditRow', () => {
	...
	it('firmadan çıkışta (returned null) kayıt üretmez', () => {
		expect(buildLoginAuditRow(ctx({ context: { session: {}, returned: null } }))).toBeNull();
	});
	...
	it('newSession yoksa session.user yedeğine düşer', () => {
		const r = buildLoginAuditRow(
			ctx({ context: { session: { user: { id: 'u3', name: 'Semih' } }, returned: { id: TENANT } } })
		);
	...
	it('beklenmedik şekilde çökmez', () => {
		for (const kotu of [null, undefined, 42, 'metin', {}, { path: SET_ACTIVE_PATH }]) {
