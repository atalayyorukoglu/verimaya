...
import { userRoleSchema, type MembershipUser, type UserRole } from '@verimaya/shared';
...
export class MeService {
	...
	async resolveMembershipUser(input: ResolveMembershipInput): Promise<MembershipUser> {
		const activeOrganizationId = this.requireActiveOrganization(input);
		const membership = await this.findMembership({ ...input, activeOrganizationId });
	...
	async resolveOrganizationRole(input: ResolveMembershipInput): Promise<UserRole> {
		const activeOrganizationId = this.requireActiveOrganization(input);
		const membership = await this.findMembership({ ...input, activeOrganizationId });
	...
	private async findMembership({
		...
		activeOrganizationId,
		requestId
	}: ResolveMembershipInput & { activeOrganizationId: string }): Promise<MembershipRow> {
